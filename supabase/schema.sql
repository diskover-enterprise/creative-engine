-- Creative Engine — schema.sql
-- Run this whole file in the Supabase SQL Editor. It's fully idempotent: the
-- legacy Phase 1-5 bootstrap (Brand -> Product -> Campaign -> Concept ->
-- Creative) only ever runs on a brand-new database, since Phase 6 merged
-- Brand/Product/Campaign into one Campaign entity and Phase 7 renamed
-- Concept/Creative to Ad Set/Ad. On a database that already has an `ad_sets`
-- table, the entire legacy block below is skipped -- re-running this file
-- after that point only applies whatever's new (currently Phase 8).
...
-- Legacy Phase 1-5 bootstrap. Only runs on a database that has never been
-- migrated to the merged Campaign / Ad Set / Ad model (see Phase 6/7 below).
do $$
begin
  if not exists (select 1 from information_schema.tables where table_schema = 'public' and table_name = 'ad_sets') then
    ...
    -- Concepts hold a structured creative brief. `generated_prompt` is
    -- computed deterministically (see lib/promptTemplate.ts) from these
    -- fields plus the parent Brand's voice/style and Product's
    -- audience/benefits -- no AI model is called to produce it.
    ...
    -- Creatives can be manually uploaded ('manual_upload') or produced by an
    -- AI provider ('ai_generated'); `provider` and `generation_prompt` are
    -- only set for the latter (a snapshot of the prompt actually used, since
    -- the parent Concept's generated_prompt can change later).
    ...
    -- Async generation job tracking, so a Route Handler can poll fal.ai in
    -- short-lived requests instead of one request blocking until it's done
    -- (which would time out on serverless deployments for slower
    -- generations).
...
-- Storage buckets -- "on conflict do nothing" makes these safe to run every
-- time regardless of migration state.
...
-- Creative Engine — Phase 8 schema addition (AI ad copy, UGC clip scripts,
-- Shotstack stitching). `headline`/`caption` hold the AI-written marketing
-- copy generated for every completed image Ad. `ad_clips` holds the 5-clip
-- UGC video script for a video Ad Set -- written up front for review, then
-- each clip is generated (via Higgsfield) only after approval, and finally
-- stitched into one Ad (via Shotstack).
alter table ads add column if not exists headline text;
alter table ads add column if not exists caption text;
...
create table if not exists ad_clips (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (ad_set_id, clip_number)
);

create index if not exists ad_clips_ad_set_id_idx on ad_clips (ad_set_id);

drop trigger if exists ad_clips_set_updated_at on ad_clips;
create trigger ad_clips_set_updated_at
before update on ad_clips
for each row execute function set_updated_at();

alter table ad_clips enable row level security;

-- Links a generation_jobs row to the specific clip it's generating, so the
