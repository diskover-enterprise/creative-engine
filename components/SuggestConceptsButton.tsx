...
import { suggestConceptsForCampaign } from "@/app/campaigns/actions";
import { saveSuggestedConcepts } from "@/app/concepts/actions";
import type { ConceptSuggestion } from "@/lib/anthropic";

type Phase = "idle" | "loading" | "previewing" | "saving" | "failed";

export default function SuggestConceptsButton({ campaignId }: { campaignId: string }) {
  ...
  async function handleSuggest() {
    ...
    const result = await suggestConceptsForCampaign(campaignId);
    if ("error" in result) {
  ...
  async function handleSave() {
    ...
    const result = await saveSuggestedConcepts(campaignId, chosen);
  ...
  if (phase === "previewing" || phase === "saving") {
    ...
    return (
      <div className="flex flex-col gap-3 rounded-lg border border-black/10 p-4 dark:border-white/10">
        <p className="text-sm font-medium">Review suggested concepts</p>
  ...
  return (
    <div className="flex flex-col items-start gap-2">
      ...
      >
        {phase === "loading" ? "Generating suggestions..." : "Suggest Concepts (AI)"}
      </button>
    </div>
  );
}
